...
window.addEventListener('load', () => {
    ...
    document.querySelectorAll('.article-content nav.wiki-nav > div > div').forEach(div => {
    ...
    const print = paths => {
        return paths.reduce((acc,curr,idx,arr) => {
            if (typeof(curr) !== 'string') {
                ...
                const filename = curr.file ? `<li ><a href="${curr.href}">${curr.file}</a>${expander}</li>` : `<li>${curr.path}${expander}</li>`;
                return `${filename}${paths ? `<ul class="nodisplay">${paths}</ul>` : ''}` + acc
            }
            return acc
        }, '')
    }
    html = `<a class="wiki-button wiki-nav-button">Wiki Menu</a><ul class="nodisplay">${print(Object.values(hierarchy))}</ul>`
    ...
    if (breadcrumbs) Array.from(breadcrumbs).forEach(span => {
        if (!span.dataset.path) return;
        if (!Object.keys(getByString(hierarchy, span.dataset.path)).includes('file')) {
